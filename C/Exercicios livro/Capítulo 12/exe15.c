...
//esse bloco cria o tamanho da string que irá receber somente as letras do arquivo
  ...
  //cria uma string dinâmicamente          // letras + 1 ('\0')
  char *string_com_letras_do_arquivo = malloc((indice_da_string + 1) * sizeof(char));


 //cada índice da string recebe uma letra do arquivo
  caractere = fgetc(arquivo);
  while(caractere != EOF){
    if ((caractere >= 64 && caractere <= 90) || (caractere >= 97 && caractere <= 122)){
...
void ocorrenciaDaLetraNoArquivo(char caractere, char *string_com_letras_do_arquivo){
  ...
  //compara se há a letra na string e conta quantas vezes ela aparece na string
  for (i = 0; i < strlen(string_com_letras_do_arquivo); i++){
    if (string_com_letras_do_arquivo[i] == caractere){
      conta_ocorrencia_letra++;
      string_com_letras_do_arquivo[i] = '.';// a letra é substituida por ponto
    }   // para não ficar  repetindo a mesma letra quando ela for verificada de novo
  }
